...
def main():
    ...
    p.add_argument('--data', type=str, default='huggiunface', help= 'hugginface or path to local jsonl (unused in demo)')
    ...
    model = GPTModern(vocab_size = col.vocab_size, block_size = args.block_size, n_layer=args.n_layer, n_head = args.n_head, n_embd = args.n_embd, use_rmsnorm = True, use_swiglu = True, rope =True).to(device)
    ...
    while step < args.steps:
        ...
        if not batch:
            #restart curriculum 
            #cur = list(LenghtCurriculum(tuples));
            i=0
            continue
