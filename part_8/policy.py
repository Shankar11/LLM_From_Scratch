...
try:
    from model_utilites.model_modern import GPTModern # user-custom path
except Exception:
...
class PolicyWithValue(nn.Module):
    ...
    NOTE: For simplicity we palce value head on top of LM logits (vocab +1).
    ...
    def __init__(self, vocab_size:int, block_size: int, n_layer=4, n_head=4, n_embd=256,
        ...
        self.lm = GPTModern(vocab_size=vocab_size, block_size=block_size, n_layer=n_layer,
                            n_head=n_head, n_embd=n_embd, use_rmsnorm= use_rmsnorm, use_swiglu=use_swiglu,
