    ...
    if load_dataset is not None:
        try:
            ...
            for row in ds:
                ...
                rj = str(row.get("rejeced", "")).strip()
                if ch and rj:
                    items.append(PrefExample(prompt="", chosen=ch, rejected=rj))
        except Exception:
            print("Failed to load Anthropoc/hh-rlhf dataset. Using fallback toy pairs")
            pass
    ...
    if not items:
        ...
        items = [
            ...
            PrefExample("Give two uses of attention in transformers.",
                        ...
                        "It remembers all past wods exactly without any computation"),
        ]
    return items