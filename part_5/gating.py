...
class TopKGate(nn.Module):
    """
    Top-k softmax gating with Switch-style load-balancing aix loss.
    Args:
        dim: input hidden size
        n_expert: number of experts
        k: number of experts tp route per token(1 or 2 typical)
    Returns:
    (indices, weights, aux_loss) where
            indices: (S,k) long, expert ids for each token
            weights: (s,k) float, gate weights (sum <= 1per token)
    ...
    def forward(self, x:torch.Tensor):
        ...
        #load: fraction of token assigned as primary (top-1 hard assignment)
        hard1 = topk_idx[:,0]
