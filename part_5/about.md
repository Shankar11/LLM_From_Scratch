...
-**Load balancing**: encourage uniform  expert usage. A common aux loss ( Switch Transformer) is 
    `L-aux = E * Σ (importance * load)` where:
...
- Here we keep everything  **on one device** for simplicity. Dispatch simulated with indexing/masking.
...
**Milestone for this part:** integrate this MoE layer in place of a dense feed-forward in a transformer block and compare efficiency/accuracy trade-offs - even with a toy attention blobk if you wish.