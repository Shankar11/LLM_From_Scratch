...
class CausalSelfAttentionModern(nn.Module):
    def __init__(self, n_embd: int, n_head: int, dropout:  float=0.0,
        ...
        self.n_head = n_head
        self.n_kv_head = n_kv_head or n_head    #GQA defaults to MHA
        assert self.n_head % self.n_kv_head ==0 , "n_head must be a multiple of n_kv_head (GQA grouping)"
    ...
    def forward(self, x:torch.Tensor, kv_cache: KVCache | None = None, start_pos: int = 0):
        """
        x:(B, T, C). If kv_cache given, we assume generation( T small ,oftern 1).
        """
        B, T, C = x.shape
        ...
        k= self.wk(x).view(B, T, self.n_kv_head , self.d_head).transpose(1,2) #(B,Hk, D)
        v= self.vk(x).view(B, T, self.n_kv_head, self.d_head).transpose(1,2)  #(B,Hk, D)
        ...
        #Scaled dot production attention  (Pytorch scales internally)
