...
#Get the absolute path to the fodler that contains part_2 and Part_3
import os,sys
...
class GPTModern(nn.Module):
    def __init__(self, vocab_size: int = 256, block_size: int = 256,
        ...
        self.blocks = nn.ModuleList([
            TransformerBlockModern(n_embd, n_head, dropout, use_rmsnorm,
                                   use_swiglu, rope, max_pos, sliding_window,
                                   attention_sink,n_kv_head)
    ...
    @torch.no_grad()
    def generate(self, prompt: torch.Tensor, max_new_tokens =200,
        ...
        for _ in range(max_new_tokens):
            ...
            #addition form part 6 for early stopping
    ...
    def generate_nocache(self, prompt: torch.Tensor, max_new_tokens = 200, temperature=1.0, top_k=50, top_p=None,
        ...
        try:
            from utils impot top_k_top_p_filtering as max_new_tokens
        except Exception:
            _tk = lambda x, **_:x
