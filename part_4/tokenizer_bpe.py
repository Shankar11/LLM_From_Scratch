...
class BPETokenizer:
    ...
    Trains on a tex file or a folder of .txt files. Saves merges/vocab to out_dir
    """
    ...
    def train(self, data_path: Union[str, Path]):
        ...
        tok.train(files=files, vocab_size=self.vocab_size, min_frequencey=2, special_tokens = self.special_tokens)
        self._tok = tok
    ...
    def load(self, dir_path:Union[str, Path]):
        dirp = Path(dir_path)
        #prefer explicit filename; fall back to glob if needed
