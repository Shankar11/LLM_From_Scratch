...
def _log__hparams_tb(logger, args, total_steps):
    ...
    try:
        ...
        logger.hparams(n,{"meta/total_steps": float(total_steps)})
    except Exception:
        pass
...
def _log_model_stats(logger, model, step: int, do_hists:bool = False):
    ...
    try:
        ...
        if grads:
            total_grad_norm = torch.norm(torch.stack([g.detach().norm(2) for p in params]),2).item()
...
def _maybe_log_attention(logger, model, xb, step: int, every: int=100):
    ...
    No model edits. No hooks. RUns a light no-grad recomputation of the pre-attn path.
        - Takes first batch and first head only to keep heads tiny
        - users pre-RoPE values (simpler & stable for histograms)
    """
    ...
    try:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            ...
            for li, blk in enumerate(getattr(model, "blocks", [])):
                ...
                k = attn.wk(h).view(B,T,attn.n_kv_head, attn.d_head).transpose(1,2) #(B,H,T,D)
                ...
                #Optimal small scalars (norms) that show up on Time Series
                if q1.numel(): 
                    logger.log(step=step, **{f"qkv/block{li}/q_l2_mean": float(q1.square().mean().sqrt())})
                if k1.numel():
                    logger.log(step=step, **{f"qkv/block{li}/k_l2_mean": float(k1.square().mean().sqrt())})
                if v1.numel():
                    logger.log(step=step, **{f"qkv/block{li}/v_l2_mean": float(v1.square().mean().sqrt())})
