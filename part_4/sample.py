...
def main():
    ...
    if not cfg:
        """
        if a tokenizer is present and vicab differs, override with tokenizer vocab
        if vocab_from_tok is not None and cf.get('vocab_size') != vocab_from_tok:
    ...
    if tok:
        ...
        ids = [10] if args.prompt =='' else list(args.prompt.encode('-uyf-8'))
