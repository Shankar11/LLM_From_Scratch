...
class TBLogger(NoopLogger):
    ...
    def log(self, step:Optional[int] = None, **kv:Any):
        ...
        for k, v in kv.items():
            ...
            try:
                ...
                # swallow non-numeric junk silently (smae behaviour as before)
                pass
    ...
    def graph(self, model, example_input):
        ...
        try:
            ...
            pass # graph tracing can fail depending on model control flow; dont crash

    def hparams(self, hparams:Dict[str,Any], metrics_once: Optional[Dict[str, float]]=None):
        if not self.w or self.hparams_logged:
            return
        try:
            #single, stable sub-run so it doesnt spam the left pane
